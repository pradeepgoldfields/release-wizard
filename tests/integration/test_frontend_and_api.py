...
def test_get_product(client):
    prods = client.get("/api/v1/products").get_json()
    pid = prods[0]["id"]
...
def test_create_environment(client):
    pid = client.get("/api/v1/products").get_json()[0]["id"]
...
def test_update_compliance_score(client):
    pid = client.get("/api/v1/products").get_json()[0]["id"]
    plid = client.get(f"/api/v1/products/{pid}/pipelines").get_json()[0]["id"]
    r = client.post(
...
def test_create_and_list_pipeline_run(client):
    ...
    runs = client.get(f"/api/v1/pipelines/{plid}/runs").get_json()
    assert len(runs) >= 1
...
def test_update_pipeline_run_status(client):
    pid = client.get("/api/v1/products").get_json()[0]["id"]
    plid = client.get(f"/api/v1/products/{pid}/pipelines").get_json()[0]["id"]
    run_id = client.get(f"/api/v1/pipelines/{plid}/runs").get_json()[0]["id"]
